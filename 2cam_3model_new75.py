...
def program_camera1_FH(model,size,conf,regno):
    ...
    if len(filenames) == 0:
        ...
        for filename1 in filenames:
            ...
            show1 = cv2.resize(show1, (image_size_camera1,image_size_camera1), interpolation = cv2.INTER_AREA)
                        ...
                        if model1.names[i1] in names1:
                            if model1.names[i1] == 'bui_chi':
                                ...
                                if toa_do_y < 300 and toa_do_x < 1190:
                                    ...
                                    break 
                                elif toa_do_y > 900 and toa_do_x < 1190:
                                    print('buichi tay choi duoi')
                                    flag = True
                                    cv2.putText(show1, 'NG',(result_height_display,result_height_display),cv2.FONT_HERSHEY_COMPLEX, 3,(0,0,255),5)
                                    cv2.putText(show1, model1.names[i1],(5,30*k),cv2.FONT_HERSHEY_COMPLEX, 1,(0,0,255),0)
                                    myresult1 = 1      
                                    ...
                                    break                                        
                                elif toa_do_x < 300 and toa_do_y < 1190:
                                    print('buichi chau dien trai')
                                    flag = True
                                    cv2.putText(show1, 'NG',(result_height_display,result_height_display),cv2.FONT_HERSHEY_COMPLEX, 3,(0,0,255),5)
                                    cv2.putText(show1, model1.names[i1],(5,30*k),cv2.FONT_HERSHEY_COMPLEX, 1,(0,0,255),0)
                                    myresult1 = 1  
                                    ...
                                    # write_plc_keyence(register_ng,1)
                                    break
                                elif toa_do_x > 900 and toa_do_y < 1190:
                                    print('buichi chau dien phai')
                                    flag = True
                                    cv2.putText(show1, 'NG',(result_height_display,result_height_display),cv2.FONT_HERSHEY_COMPLEX, 3,(0,0,255),5)
                                    cv2.putText(show1, model1.names[i1],(5,30*k),cv2.FONT_HERSHEY_COMPLEX, 1,(0,0,255),0)
                                    myresult1 = 1    
            ...
            if myresult1 == 0:
                ...
                cv2.putText(show1, 'NG',(1000,100),cv2.FONT_HERSHEY_COMPLEX, 3,(0,0,255),5)
                window['result_cam1'].update(value= 'NG', text_color='red')  
...
def program_camera2_FH(model,size,conf,file):
    ...
    myitem +=1
    show_2 = cv2.resize(show2, (image_width_display_c2,image_height_display_c2))
    imgbytes2 = cv2.imencode('.png',show_2)[1].tobytes()

    show_2_layout = cv2.resize(show2, (600,450))
    imgbytes2_layout = cv2.imencode('.png',show_2_layout)[1].tobytes()

    print("cd: ", cd)
    if cd==1:
        window['cd_1'].update(data= imgbytes2_layout)
        window['cd_11'].update(data= imgbytes2)
        
    else:
        window['cd_2'].update(data= imgbytes2_layout)
        window['cd_22'].update(data= imgbytes2)
...
def program_camera3_FH(model,size,conf,file):
    ...
    show_3 = cv2.resize(show3, (image_width_display_c2,image_height_display_c2))
    imgbytes3 = cv2.imencode('.png',show_3)[1].tobytes()

    show_3_layout = cv2.resize(show3, (600,450))
    imgbytes3_layout = cv2.imencode('.png',show_3_layout)[1].tobytes()

    if tc==1:
        window['tc_1'].update(data= imgbytes3_layout)
        window['tc_11'].update(data= imgbytes3)
    else:
        window['tc_2'].update(data= imgbytes3_layout)
        window['tc_22'].update(data= imgbytes3) 
...
def make_window(theme):
    ...
    layout_main = [
        ...
        #1
        sg.Frame('',[
            [sg.Text(' '*2,font=('Helvetica',20), justification='center'),
            sg.Image(filename='', size=(image_size_camera1,image_size_camera1),key='image1',background_color='black',)],
            [sg.Frame('',
            [
    ...
    #window.bind('<Configure>',"Configure")image_width_display
    window.Maximize()

    return window


image_width_display = int(1000*0.8)
image_height_display = int(600*0.8)
...
image_size_camera1 = int(600*0.8)
...
try:
    while True:
        ...
        if changed==1 or event == 'choose_model' or started == 1:
            ...
            window = make_window(theme)
            if mychoose=='3':
                window['choosemodel_running'].update(value = 'MÁY NQHCVN M100 - ĐẾ ĐEN')
            if mychoose=='1':
                window['choosemodel_running'].update(value = 'MÁY NQHCVN M100 - ĐẾ TRẮNG')
            #print(weight1)
        ...
        try:
            ...
            # if read_D == 1:   
                ...
                # write_plc_keyence(17452,1) # hoan tat
            ...
            folder1 = glob('C:/FH/camera2/CHAU1/**/Input0_Camera0.jpg')
            folder2 = glob('C:/FH/camera2/CHOI1/**/Input0_Camera0.jpg')
            folder3 = glob('C:/FH/camera2/CHAU2/**/Input0_Camera0.jpg')
            folder4 = glob('C:/FH/camera2/CHOI2/**/Input0_Camera0.jpg')
            ...
            if myitem >= 4:
                ...
                if len(all_error) == 0:
                    ...
                    # write_plc_keyence(430,int(values['OK_PLC_2']) * int(values['OK_PLC_3']))
                    window['result_cam2'].update(value= 'OK', text_color='green')
                if len(all_error) == 1:
                    if int(all_error[0]) == 9:
                        # write_plc_keyence(17470,2)
                        # write_plc_keyence(int(all_error[0]),2)
                        print('eror')
                    else:
                    ...
                    # write_plc_keyence(430,int(all_error[0]))
                ...
                if len(all_error) >= 2:
                    for error in all_error:
                        if error == 9:
                            ...
                            print('eror1')
                        # write_plc_keyence(error,1) # nhieu hang muc
                    
                    window['result_cam2'].update(value= 'NG*', text_color='red')
                    print('all_error', all_error)
                        # write_plc_keyence(460 + error*2 ,1)
        ...
        #         # bomau = glob('D:/CHECK/' + maude + '/Cam1/NG/*.jpg')
        ...
        if event == 'check_model2' and values['check_model2'] == True and values['Tay_choi'] == True :
            ...
            if os.listdir(directory2) == []:
                ...
                for path2 in bomau:
                    ...
                    cv2.putText(show2, ten,(20,20),cv2.FONT_HERSHEY_COMPLEX, 1,(0,255,0),1)
                    imgbytes2 = cv2.imencode('.png',show2)[1].tobytes()
                    window['tc_11'].update(data= imgbytes2)
        ...
        if event == 'Pic1':
            ...
            if dir_img1 not in ('',None):
                ...
                img1_resize = pic1.resize((image_size_camera1,image_size_camera1))
        ...
        if event == 'Pic2':
            ...
            if dir_img2 not in ('',None):
                ...
                img2_resize = pic2.resize((image_width_display_c2,image_height_display_c2))
        ...
        if event == 'Detect1':
            ...
            try:               
                ...
                if myresult1 == 0:
                    ...
                    window['result_cam1'].update(value= 'OK', text_color='green')
                show_1 = cv2.resize(show1, (image_size_camera1,image_size_camera1))
