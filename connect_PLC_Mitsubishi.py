...
def read_word(name, number_name):
    ...
    frame1 = {'data': [1,0xff,0x0A,0,number_name,0,0,0x20,(ord(name)),1,0]}
    dummy =[]
    mydata = bytes([1])
    if number_name >255:
        for field in list(frame1.values()):
            for item in field[1:]:
                if item == number_name:
                    number_name = number_name.to_bytes(2, 'little')
                    mydata += number_name
                else:
                    a = []
                    a.append(item)
...
def write_word(name, number_name, data):
    ...
    frame1 = {'data1': [3,0xff,0x0A,0,number_name,0,0,0x20,(ord(name)),
                           1,0,int(doiso1,2),int(doiso2,2)]}
    dummy =[]
    mydata = bytes([3])
    if number_name >255:
